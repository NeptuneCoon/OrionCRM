...
namespace Orion.CRM.DataAccess
{
    public class ResourceDataAdapter : DataAdapter
    {
        ...
        {
            if (string.IsNullOrEmpty(resourceIds)) return null;

            SqlMapDetail mapDetail = (SqlMapDetail)SqlMapFactory.GetSqlMapDetail("ResourceDomain", "GetResourceGroupsByResourceIds").Clone();
            mapDetail.OriginalSqlString = mapDetail.OriginalSqlString.Replace("$ResourceIds", resourceIds);
            IEnumerable<Entity.ResourceGroup> resourceGroups = SqlMapHelper.GetSqlMapResult<Entity.ResourceGroup>(mapDetail);
        ...
        public IEnumerable<Entity.ResourceUser> GetResourceUsersByResourceIds(string resourceIds)
        {
            ...
            SqlMapDetail mapDetail = (SqlMapDetail)SqlMapFactory.GetSqlMapDetail("ResourceDomain", "GetResourceUsersByResourceIds").Clone();
            mapDetail.OriginalSqlString = mapDetail.OriginalSqlString.Replace("$ResourceIds", resourceIds);
            IEnumerable<Entity.ResourceUser> resourceGroups = SqlMapHelper.GetSqlMapResult<Entity.ResourceUser>(mapDetail);
        ...
        {
            if (resources == null || resources.ToList().Count == 0) return 0;
            bool result = SqlMapHelper.ExecuteBatchInsert<Entity.ResourceEntity>("ResourceDomain", "BatchInsertResource", resources);
            return result ? resources.ToList().Count : 0;
        }
        ...
        {
            if (resourceOrgs == null || resourceOrgs.ToList().Count == 0) return 0;
            bool result = SqlMapHelper.ExecuteBatchInsert<Entity.ResourceOrganization>("ResourceDomain", "BatchInsertResourceOrg", resourceOrgs);
            return result ? resourceOrgs.ToList().Count : 0;
        }
        ...
        {
            if (resourceProjects == null || resourceProjects.ToList().Count == 0) return 0;
            bool result = SqlMapHelper.ExecuteBatchInsert<Entity.ResourceProject>("ResourceDomain", "BatchInsertResourceProject", resourceProjects);
            return result ? resourceProjects.ToList().Count : 0;
        }
        ...
        {
            bool result = SqlMapHelper.ExecuteBatchInsert<Entity.ResourceUser>("ResourceDomain", "ResourceUserBatchInsert", resourceUsers);
            return result;
        }
    }
}
